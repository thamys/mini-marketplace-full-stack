...
export function CartDrawer() {
  ...
  const renderCartContent = () => {
    if (checkingStock) {
      return (
        <div className="flex flex-col gap-3 px-4 sm:px-5 py-4" aria-label="Verificando estoque...">
          {[1, 2].map((i) => (
    ...
    return (
      <ul className="space-y-3 px-4 sm:px-5 py-4 pb-4" aria-label="Itens no carrinho">
        {items.map((item) => {
    ...
    <>
      ...
      >
        ...
        {totalItems > 0 && (
          <span
            key={badgeKey}
            className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#7BFFAF] text-[10px] font-bold text-[#0F0B1A] badge-bounce"
            aria-hidden="true"
          >
      ...
      {createPortal(
        <>
          ...
          <aside
            className={cn(
              'fixed top-0 right-0 bottom-0 z-50 w-[90vw] sm:w-full max-w-[420px] bg-background drawer-spring',
        ...
        {/* Header */}
        <header className="flex items-center justify-between border-b px-4 sm:px-5 py-4 bg-background/95 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <ShoppingBag className="h-5 w-5 text-primary" />
            <h2 id="cart-drawer-title" className="text-base font-semibold">
              Carrinho
              {totalItems > 0 && (
              ...
              <div className="space-y-1.5 text-sm text-muted-foreground">
                {items.map((item) => (
                  <div key={item.productId} className="flex justify-between gap-2">
                    ...
                    <span className="shrink-0">{fmt.format(item.price * item.quantity)}</span>
                  </div>
                ))}
              </div>
